    ...
    try {
        ...
        for (const auto& result : results) {
            ProfileModel* profile = new ProfileModel(
                ...
                result.value("sex").toString(), result.value("weight").toInt(),
...
bool UserProfileController::createProfile(int userId, const QString& name,
    ...
    try {
        ...
        db.execute(
            "INSERT INTO profile (user_id, name, description, sex, weight, "
            "height, date_of_birth) VALUES (?,?,?,?,?,?,?);",
...
bool UserProfileController::createProfile(ProfileModel* profile) {
    try {
        db.execute(
            "INSERT INTO profile (user_id, name, description, sex, weight, "
            "height, date_of_birth) VALUES (?,?,?,?,?,?,?);",
            {profile->getUserId(), profile->getName(), profile->getDesc(),
             profile->getSex(), profile->getWeight(), profile->getHeight(),
             profile->getDobString()});
        return true;
    } catch (const std::exception& e) {
...
bool UserProfileController::updateProfile(int profileId, int userId,
    ...
    try {
        QString dobString = dob.toString("yyyy-MM-dd");
        db.execute(
            "UPDATE profile SET user_id = ?, name = ?, description = ?, sex = "
            "?, weight = ?, height = ?, date_of_birth = ? WHERE profile_id = "
            "?;",
...
bool UserProfileController::updateProfile(ProfileModel* profile) {
    try {
        db.execute(
            ...
            {profile->getUserId(), profile->getName(), profile->getDesc(),
             profile->getSex(), profile->getWeight(), profile->getHeight(),
             profile->getDobString(), profile->getId()});
        ...
        for (const auto& result : results) {
            ScanModel* scan =
                new ScanModel(result.value("scan_id").toInt(),
                              ...
                              result.value("f5_gall_bladder").toInt(),
                              result.value("f5_gall_bladder_r").toInt(),
