 ...
 * @param profile a reference to the profile object to be populated with the
 * result
 * @return true if the profile was found
 */
bool UserProfileController::getProfileByName(int userId, const QString& name,
                                             ProfileModel& profile) const {
    QList<QMap<QString, QVariant>> results;

    try {
        db.query("SELECT * from profile WHERE user_id = ? AND name = ?;",
                 {userId, name}, results);
        ...
        profile.setDob(result.value("dob").toDate());
 ...
 */
bool UserProfileController::getProfiles(
    int userId, QVector<ProfileModel*>& profiles) const {
    QList<QMap<QString, QVariant>> results;
    ...
    try {
        ...
        for (const auto& result : results) {
            ProfileModel* profile = new ProfileModel(
                ...
                result.value("name").toString(), result.value("sex").toString(),
                result.value("weight").toInt(), result.value("height").toInt(),
                result.value("dob").toDate());
 ...
 * @return true if the operation was successful
 */
bool UserProfileController::createProfile(int userId, const QString& name,
                                          const QString& sex, int weight,
                                          int height, const QDate& dob) {
    try {
        db.execute(
            "INSERT INTO profile (user_id, name, sex, weight, height, "
            "date_of_birth) VALUES(?,?,?,?,?,?,?);",
            {userId, name, sex, weight, height, dob});
        return true;
    } catch (const std::exception& e) {
...
bool UserProfileController::createProfile(ProfileModel* profile) {
    try {
        db.execute("INSERT INTO profile (user_id, name) VALUES (?,?,?);",
                   {profile->getUserId(), profile->getName()});
        return true;
    } catch (const std::exception& e) {
 ...
 * @return true if the operation was successful
 */
bool UserProfileController::updateProfile(int profileId, int userId,
                                          const QString& name,
                                          const QString& sex, int weight,
                                          int height, const QDate& dob) {
    try {
        db.execute(
            "UPDATE profile SET user_id = ?, name = ?, sex = ?, weight = ?, "
            "height = ?, date_of_birth = ? WHERE profile_id = ?;",
            {userId, name, sex, weight, height, dob, profileId});
        return true;
    } catch (const std::exception& e) {
...
bool UserProfileController::updateProfile(ProfileModel* profile) {
    try {
        db.execute(
            "UPDATE profile SET user_id = ?, name = ? WHERE profile_id = ?;",
            {profile->getUserId(), profile->getName(), profile->getId()});
...
bool UserProfileController::deleteProfile(ProfileModel* profile) {
    try {
        db.execute("DELETE FROM profile WHERE profile_id = ?;",
                   {profile->getId()});
        return true;
    } catch (const std::exception& e) {
        qCritical() << "Failed to delete profile: " << e.what();
 ...
 * @return true if the operation was successful
 */
bool UserProfileController::getProfileScans(int profileId,
                                            QVector<ScanModel*>& scans) const {
    QList<QMap<QString, QVariant>> results;
    try {
        db.query("SELECT * FROM scan WHERE profile_id = ?;", {profileId},
                 results);

        scans.clear();
        for (const auto& result : results) {
            ScanModel* scan =
                new ScanModel(result.value("scan_id").toInt(),
                              result.value("profile_id").toInt(),
